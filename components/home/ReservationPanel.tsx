...
import { reservationData } from "@/app/data/reservation";
...
export default function ReservationPanel({ locale }: ReservationPanelProps) {
  const currentLocale = locale || "ja";
  const ui = reservationData[currentLocale] || reservationData.ja;

  return (
    <section className={styles.reservationSection} id="reservation">
      <div className={styles.container}>
        
        {/* Lưới Grid thông minh thích ứng mọi loại màn hình */}
        <div className={styles.layoutGrid}>
          
          {/* CỘT TRÁI: Nội dung giới thiệu tinh tế */}
          ...
          {/* CỘT PHẢI: Thẻ dịch vụ cao cấp */}
          <div className={styles.luxuryCard}>
            
            {/* Bộ đôi nút thao tác chính */}
            <div className={styles.actions}>
              {/* Nút đặt bàn */}
              <Link className={`${styles.button} ${styles.btnPrimary}`} href="/reservation">
              ...
              {/* Nút giao hàng */}
              <Link className={`${styles.button} ${styles.btnSecondary}`} href="/delivery">
                <svg 
                  className={`${styles.btnIcon} ${styles.uberEatsIcon}`} /* Thêm class riêng uberEatsIcon */
                  fill="#0c2d1c" 
                  role="img" 
                  viewBox="0 0 24 24" 
            ...
            {/* Khu vực Gọi hotline liên hệ */}
            <div className={styles.phoneBlock}>
              <span className={styles.phoneLabel}>{ui.phoneLabel}</span>
              <a href={`tel:${ui.phoneNo.replace(/-/g, "")}`} className={styles.phoneLink}>
                <svg className={styles.phoneIcon} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5">
                  <path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-6-6 19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 4.11 2h3a2 2 0 0 1 2 1.72 12.84 12.84 0 0 0 .7 2.81 2 2 0 0 1-.45 2.11L8.09 9.91a16 16 0 0 0 6 6l1.27-1.27a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7A2 2 0 0 1 22 16.92z" />
                </svg>
                {ui.phoneNo}
              </a>
              <span className={styles.phoneNote}>{ui.phoneNote}</span>
            </div>
